...
int subregion(int **matrix, int row, int column, int i, int j, int *base, int *heigth);
...
Rectangle region(int **matrix, int row, int column) {
    ...
    for (i = 0; i < row; i++) {                                                      // cycle on the row of the matrix
        for (j = 0; j < column; j++) {                                               // cycle on the column of the matrix
            if(subregion(matrix, row, column, i, j, &l, &k) != FALSE) {
...
int subregion(int **matrix, int row, int column, int i, int j, int *base, int *heigth) {
    if(matrix[i][j] == 1) {                                                                                // if that checks if element is the superior left extreme of the rectangle
        for (*heigth = 0, *base = 0; i + *heigth < row && matrix[i + *heigth][j] == 1; (*heigth)++) {      // cycle on the row of the submatrix
            for (; j + *base < column && matrix[i + *heigth][j + *base] == 1; (*base)++) {                 // cycle on the column of the submatrix
                matrix[i + *heigth][j + *base] = 2;
            }
        }
        return 1;
    }
