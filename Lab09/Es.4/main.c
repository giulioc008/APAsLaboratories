...
int main() {
    ...
    printPlayers(lista, stdout, inventario);
    fprintf(stdout, "\n\n\t\tInventory:\n");
    ...
    switch(command){                                                                                                    // switch that regulates the operation
        case add:
            ...
            if(x != NULL) {                                                                                             // if that checks if the search has product an output
                addEquipment(next(x), inventario);
            } else {
                if(stricmp(playerCode(gamer(head)), string) == 0) {                                                     // if that checks the particular case (the node to search is the head)
        ...
        case extract:
            ...
            x = searchPlayer(head, string);
            if(x != NULL) {                                                                                             // if that checks if the search has product an output
                removeEquipment(next(x), inventario);
            } else {
                if(stricmp(playerCode(gamer(head)), string) == 0) {                                                     // if that checks the particular case (the node to search is the head)
                    removeEquipment(head, inventario);
                } else {
                    fprintf(stdout, "\tTheplayer doesn't exists\n");
                }
            }
...
int selectionMenu(List lista, Inventory inventario, menu command) {
    ...
    switch(command){                                                                                                    // switch that regulates the operation
        ...
        case stat:
            ...
            if(x != NULL) {                                                                                             // if that checks if the search has product an output
                s = stats(gamer(next(x)), inventario);
                fprintf(stdout, "The statistics of the player %s are:\n\tHealt Points: %d\n\tMP: %d\n\tAttack: %d\n\tDefense: %d\n\tMag: %d\n\tSpr: %d\n", playerName(gamer(next(x))), s.healtPoints, s.mp, s.attack, s.defense, s.mag, s.spr);
            } else if(stricmp(playerCode(gamer(head(lista))), string) == 0) {                                           // if that checks the particular case (the node to search is the head)
                s = stats(gamer(head(lista)), inventario);
                fprintf(stdout, "The statistics of the player %s are:\n\tHealt Points: %d\n\tMP: %d\n\tAttack: %d\n\tDefense: %d\n\tMag: %d\n\tSpr: %d\n", playerName(gamer(head(lista))), s.healtPoints, s.mp, s.attack, s.defense, s.mag, s.spr);
            } else {
                fprintf(stdout, "Theplayer doesn't exists\n");
            }
            break;
